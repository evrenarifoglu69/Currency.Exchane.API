...
namespace Currency.Exchange.EntityFramework.Redis
{
    public class RedisDatabaseProvider : IRedisDatabaseProvider
    {
        ...
        {
            ConnectionMultiplexer redis = ConnectionMultiplexer.Connect($"{StaticValues.CurrencyExchange_Redis_ConStr},allowAdmin=true");
            var server = redis.GetServer(StaticValues.CurrencyExchange_Redis_ConStr);
            await server.FlushDatabaseAsync(2);
        }

        private async Task WriteRedis(string cacheKey, string value)
        {
            await _connectionMultiplexerB2C.GetDatabase(2).StringSetAsync($"{_prefix}{cacheKey}", value, TimeSpan.FromDays(int.Parse(StaticValues.CurrencyExchange_Redis_Ttl)), When.Always);
        }
